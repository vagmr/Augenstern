...
int nTail;  // 蛇的长度
enum eDirection { STOP = 0, LEFT, RIGHT, UP, DOWN };  // 枚举类型表示方向
...
void Logic() {
    ...
    tailX[0] = x;  // 蛇尾跟随蛇头移动
    tailY[0] = y;
    for (int i = 1; i < nTail; i++) {  // 蛇身跟随蛇尾移动
        prev2X = tailX[i];
        prev2Y = tailY[i];
        tailX[i] = prevX;
        tailY[i] = prevY;
    ...
    for (int i = 0; i < nTail; i++)  // 如果蛇头碰到蛇身则游戏结束
        if (tailX[i] == x && tailY[i] == y)
            gameOver = true;
    if (x == fruitX && y == fruitY) {  // 如果蛇头碰到食物，则食物被吃掉，蛇长一节
        nTail++;
